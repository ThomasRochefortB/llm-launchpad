...
| --- | --- | --- | --- |
| **P1 — first — deployment jobs done (2026-09-16)** | **Track and reopen deployment operations.** Home → Operations (or Ctrl+O) retains each deployment's monitor and result independently. Back leaves the deployment running; cancellation requires confirmation and stops its resource after the current provider call returns. | Concurrent deployments have separate tracking and serialized recovery-journal updates. Quit requests cancellation for all jobs and waits for cleanup; failed cleanup retains its recovery record. Status, benchmark and storage operations are not yet included in the Operations list. | L |

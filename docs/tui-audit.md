...
| Home | Menu routing, keyboard shortcuts, auth/billing/deployment rendering and error states, deferred refresh, details panel, resume behavior, compact layout. Existing focused tests cover the panel data; the combined journey uses simulated data. Provider Billing now renders every provider from one template, so its loading, unconfigured and failure wording is asserted once across all three rather than per provider. |
...
| --- | --- | --- | --- |
| **P1 — first — deployment jobs done (2026-09-16)** | **Track and reopen deployment operations.** Home → Operations (or Ctrl+O) retains each deployment's monitor and result independently. Back leaves the deployment running; cancellation requires confirmation and stops its resource after the current provider call returns. | Concurrent deployments have separate tracking and serialized recovery-journal updates. Quit requests cancellation for all jobs and waits for cleanup; failed cleanup retains its recovery record. Since 2026-09-23 status checks, benchmarks, logs, stops and storage work are listed too (latest 20 per session) and reopen with their result. | L |
| **P1 — second — done (2026-09-12)** | **Distinguish provider failure from an empty fleet.** `_visible_rows_and_prune_scope` suppressed provider discovery errors and could return an empty or partial list as a normal refresh. | Met: `TuiApp.discover_fleet` reports each provider separately, retains the rows an unreachable provider last returned, and Home and Deployments name the provider, its error and the age of what they are showing. Actions on healthy providers are untouched. | M |
| **P1 — third — done (2026-09-23)** | **Make provider readiness explicit without delaying first paint.** Startup's `_provider_is_configured` treats an installed Modal CLI as configured; the setup copy describes authentication. | Met: Setup shows installed / checking / authenticated / failed per provider from an asynchronous check, offers each provider's command, and continues to the main menu on its own once a provider verifies. | M |
| **P2 — done** | **Keep model search available in normal terminals.** CSS hides `#fast-deploy-model-search` below 120 columns or in short terminals; `/` currently explains that it is unavailable. | `/` opens a temporary search field at 80×24 and 60×20, preserves the query through resize, and returns focus to the filtered list. | S |
| **P2 — done (2026-09-23)** | **Make form actions and validation easier to discover.** Storage pre-download has no submit button; `p` types text while an input is focused. Several deployment errors use transient notifications. | Met: Storage has a Pre-download button and `p` is not a priority binding; a refused deploy marks and focuses the field, opens Advanced if needed, and keeps the reason under the field and beside Deploy until the value changes. Clarify scaledown boundary semantics and GPU-count versus tensor-parallel constraints. | M |
| **P2 — done** | **Clarify storage provider scope.** The inventory and estimates describe Modal volumes while the application also supports Prime disks. | Label the current provider/scope explicitly, show what is unavailable for Prime, and separate provider-specific inventory and cost estimates when disk management is added. | M |
| **P2 — done (2026-09-23)** | **Use a clear final operation state.** Deployment publication emits `PUBLISHING` just before completion; the header can continue showing that transient state after the connection card appears. Several newer states have no specific status icon. | Completion replaces transitional wording with an unambiguous outcome; status, deploy, benchmark and storage results use consistent final summaries and navigation. | S |
| **P3 — done (2026-09-23)** | **Improve help and release coverage.** Help collects class bindings rather than the effective focused-widget bindings, and TUI modules remain excluded from `ty` checks. | Met: help reads the screen's effective bindings; `ty` checks the whole TUI package; source guards reject bare theme names in markup and stylesheet classes nothing sets. | M |

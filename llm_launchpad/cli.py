...
def wizard() -> None:
    ...
    perform_modal_run: bool = True
    launch_openhands: bool = False
    ...
    with _app_screen():
        ...
        if action == "openhands":
            # Submenu: setup or launch
            sub_action = inquirer.select(
                message="OpenHands",
                choices=[
                    {"name": "setup (write config.toml)", "value": "setup"},
                    {"name": "launch (uv run openhands --config-file ./config.toml)", "value": "launch"},
                ],
                default="setup",
                cycle=True,
            ).execute()
            ...
            if sub_action == "launch":
                perform_modal_run = False
                launch_openhands = True
            else:
                # Gather inputs for OpenHands config generation
                default_server = os.environ.get("SERVER_URL", "")
                server_url_in = inquirer.text(
                    message="Server URL (e.g., https://<user>--llamacpp-server-serve.modal.run)",
                    default=default_server,
                ).execute()
                if not server_url_in or not server_url_in.strip():
                    typer.echo("Server URL is required.", err=True)
                    raise typer.Exit(code=1)
                model_in = inquirer.text(message="Model name (server-side)", default="default").execute()
                output_in = inquirer.text(message="Output path", default="config.toml").execute()
                overwrite_in = inquirer.confirm(message="Overwrite if exists?", default=False).execute()
                api_key_in = inquirer.text(
                    message="API key (leave blank to use OPENAI_API_KEY or none)", default=""
                ).execute()

                effective_api_key = api_key_in or os.environ.get("OPENAI_API_KEY", "")
                base_url = server_url_in.rstrip("/") + "/v1"

                toml_contents = (
                    "# OpenHands configuration generated by llm-launchpad\n"
                    "# Learn more: https://www.all-hands.dev/ (OpenHands docs)\n\n"
                    "[core]\n"
                    "debug = false\n"
                    "disable_color = false\n"
                ...
                out_path = Path(output_in).expanduser()
                if out_path.exists() and not overwrite_in:
                    typer.echo(
                        f"Refusing to overwrite existing file: {out_path}. Re-run and enable overwrite.",
                        err=True,
                    )
                    raise typer.Exit(code=1)
                try:
                    out_path.parent.mkdir(parents=True, exist_ok=True)
                    out_path.write_text(toml_contents)
                except Exception as exc:
                    typer.echo(f"Failed to write {out_path}: {exc}", err=True)
                    raise typer.Exit(code=1)

                typer.echo(f"\n✅ Wrote OpenHands config to: {out_path}")
                typer.echo("Use it with OpenHands CLI via: --config-file ./config.toml")
                perform_modal_run = False

    # If OpenHands flow was selected, handle it outside the alternate screen
    if not perform_modal_run:
        if launch_openhands:
            typer.echo("\nLaunching OpenHands (uv run openhands --config-file ./config.toml)...")
            code = _run_command(["uv", "run", "openhands", "--config-file", "./config.toml"])
            if code != 0:
...
def openhands(
    ...
    model: str = typer.Option("default", help="Model name to use (server-side alias)"),
    output: Path = typer.Option(Path("config.toml"), help="Path to write OpenHands config.toml"),
    api_key: Optional[str] = typer.Option(
        None, help="API key to write into config (falls back to OPENAI_API_KEY, or blank)"
    ),
    overwrite: bool = typer.Option(False, help="Overwrite output file if it exists"),
) -> None:
    """Generate an OpenHands config.toml that points to your deployed server.

    Use the resulting file with OpenHands CLI via: --config-file ./config.toml
    """
    _print_banner()
    ...
    if not isinstance(server_url, str) or not server_url.strip():
        typer.echo("Error: server_url is required.", err=True)
        raise typer.Exit(code=1)
    ...
    # Compose minimal, sensible defaults for OpenHands
    toml_contents = (
        "# OpenHands configuration generated by llm-launchpad\n"
        "# Learn more: https://www.all-hands.dev/ (OpenHands docs)\n\n"
        "[core]\n"
    ...
    if output.exists() and not overwrite:
        typer.echo(f"Refusing to overwrite existing file: {output}. Use --overwrite to replace.", err=True)
        raise typer.Exit(code=1)
    try:
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text(toml_contents)
    except Exception as exc:
        typer.echo(f"Failed to write {output}: {exc}", err=True)
        raise typer.Exit(code=1)

    typer.echo(f"✅ Wrote OpenHands config to: {output}")
    typer.echo("Use it with OpenHands CLI via: --config-file ./config.toml")
